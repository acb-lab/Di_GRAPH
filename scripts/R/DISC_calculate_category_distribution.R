#### R script to calculate valid inter-chromosomal discordant pairs category distribution
...
discordant_reads_count_files <- list.files(
  path = root_dir,
  pattern = paste0(sample, ".*inter_discordant_pairs_unique_processed_valid_category_distribution\\.tsv$"),
  recursive = TRUE,
  full.names = TRUE
)
