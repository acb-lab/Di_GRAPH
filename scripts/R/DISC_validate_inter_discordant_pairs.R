...
if (length(args) != 9) {
  ...
  log_step("Usage: script.R <strain> <sample> <experiment> <file1> <file2> <file3> <fileoriginal> <filecontrol> <category_path>")
...
path_to_file_3 <- args[6]
path_to_file_original <- args[7]
path_to_file_control <- args[8]
category_path   <- args[9]
strain <- sub("/$", "", strain)
...
log_step(paste("file3:", path_to_file_3))
