...
LASTMODIF="2026-03-06"
VERSION="0.3.1"
MODIFICATIONS="1 muestra"
...
for subdir in "$MYWD"*/; do
  ...
  # Loop through sample prefixes (T0, T1, T2, etc.)
    ...
    # Calculate average coverage for each sample (if only one replicate, average value is the same)
      # === AFTER E1 is processed ===
      for chr in CHRIII CHRV; do
        # Average general CHRIII and CHRV coverage (Only for 75nt reads)
        if [[ -f "${subdir}/${sample}_E1_75nt_${chr}.tsv" ]]; then
          LC_NUMERIC=C awk -F'\t' '
            FILENAME == ARGV[1] {
              a[FNR] = $2 + 0
              chr[FNR] = $1
        ...
        if [[ -f "${subdir}/${sample}_E1_75nt_${chr}_MATa.tsv" ]]; then
          LC_NUMERIC=C awk -F'\t' '
            FILENAME == ARGV[1] {
              a[FNR] = $2 + 0
              chr[FNR] = $1
      ...
      if [[ -f "${subdir}/${sample}_E1_${chr}_18nt_ordered.tsv" ]]; then
        LC_NUMERIC=C awk -F'\t' '
            FILENAME == ARGV[1] {
              a[FNR] = $2 + 0
              chr[FNR] = $1
        ...
        > "${subdir}/${sample}_${chcondar}_18nt_ordered_avg.tsv"
      else
