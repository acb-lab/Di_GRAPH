    ...
    for experiment in E1 E2 E3; do
      ...
      if [[ -f "$file1" && -f "$file2" ]]; then
        ...
        Rscript - <<EOF
          ...
          path_to_categories_near_next_file <- file.path(category_path, "PMV_categories_near_extended_next.tsv")
          path_to_categories_near_prev_file <- file.path(category_path, "PMV_categories_near_extended_prev.tsv")
          path_to_categories_near_next_next_file <- file.path(category_path, "PMV_categories_near_extended_next_next.tsv")
          path_to_categories_near_prev_prev_file <- file.path(category_path, "PMV_categories_near_extended_prev_prev.tsv")
          ...
          df_categories_near_features_next <- read_tsv(path_to_categories_near_next_file,col_names = TRUE, show_col_types = FALSE)
          df_categories_near_features_prev <- read_tsv(path_to_categories_near_prev_file,col_names = TRUE, show_col_types = FALSE)
          df_categories_near_features_next_next <- read_tsv(path_to_categories_near_next_next_file,col_names = TRUE, show_col_types = FALSE)
          df_categories_near_features_prev_prev <- read_tsv(path_to_categories_near_prev_prev_file,col_names = TRUE, show_col_types = FALSE)
          ...
          prepare_for_blast <- function(df_chr_AB_filtered_file_X_ID) {
                ...
                select(Read_name_ID, Sequence, Feature_name_B, Feature_name_B_prev, Feature_name_B_next, Feature_name_B_prev_prev, Feature_name_B_next_next, Feature_name_B_prev_prev_prev, Feature_name_B_next_next_next)
                return(blast_df)
           }
          ...
          df_chr_AB_filtered_file_1_ID <- df_chr_AB_filtered_file_1_nocontrols %>%
            ...
            ungroup() %>%
            select(-pair_id) %>% 
            left_join(., df_categories_near_features, by = "Feature_name_B", relationship = "many-to-many") %>% 
            left_join(., df_categories_near_features_next, by = "Feature_name_B_next", relationship = "many-to-many") %>% 
            left_join(., df_categories_near_features_prev, by = "Feature_name_B_prev", relationship = "many-to-many")  %>% 
            left_join(., df_categories_near_features_next_next, by = "Feature_name_B_next_next", relationship = "many-to-many") %>% 
            left_join(., df_categories_near_features_prev_prev, by = "Feature_name_B_prev_prev", relationship = "many-to-many")
          ...
          blast_df_file_1_prev_prev <- prepare_for_blast(df_chr_AB_filtered_file_1_ID) %>% select(Read_name_ID, Sequence, Feature_name_B_prev_prev)
          blast_df_file_1_next_next <- prepare_for_blast(df_chr_AB_filtered_file_1_ID) %>% select(Read_name_ID, Sequence, Feature_name_B_next_next)
          blast_df_file_1_prev_prev_prev <- prepare_for_blast(df_chr_AB_filtered_file_1_ID) %>% select(Read_name_ID, Sequence, Feature_name_B_prev_prev_prev)
          blast_df_file_1_next_next_next <- prepare_for_blast(df_chr_AB_filtered_file_1_ID) %>% select(Read_name_ID, Sequence, Feature_name_B_next_next_next)
          ...
          write_tsv(blast_df_file_1_prev_prev, file = paste0(strain, "/", "T0", "_", experiment, "_inter_discordant_pairs_unique_processed_blast_prev_prev.tsv"))
          write_tsv(blast_df_file_1_next_next, file = paste0(strain, "/", "T0", "_", experiment, "_inter_discordant_pairs_unique_processed_blast_next_next.tsv"))
          write_tsv(blast_df_file_1_prev_prev_prev, file = paste0(strain, "/", "T0", "_", experiment, "_inter_discordant_pairs_unique_processed_blast_prev_prev_prev.tsv"))
          write_tsv(blast_df_file_1_next_next_next, file = paste0(strain, "/", "T0", "_", experiment, "_inter_discordant_pairs_unique_processed_blast_next_next_next.tsv"))
          ...
          blast_df_file_2_prev_prev <- prepare_for_blast(df_chr_AB_filtered_file_2_ID) %>% select(Read_name_ID, Sequence, Feature_name_B_prev_prev)
          blast_df_file_2_next_next <- prepare_for_blast(df_chr_AB_filtered_file_2_ID) %>% select(Read_name_ID, Sequence, Feature_name_B_next_next)
          blast_df_file_2_prev_prev_prev <- prepare_for_blast(df_chr_AB_filtered_file_2_ID) %>% select(Read_name_ID, Sequence, Feature_name_B_prev_prev_prev) 
          blast_df_file_2_next_next_next <- prepare_for_blast(df_chr_AB_filtered_file_2_ID) %>% select(Read_name_ID, Sequence, Feature_name_B_next_next_next)
          ...
          write_tsv(blast_df_file_2_prev_prev, file = paste0(strain, "/", sample, "_", experiment, "_inter_discordant_pairs_unique_processed_blast_prev_prev.tsv"))
          write_tsv(blast_df_file_2_next_next, file = paste0(strain, "/", sample, "_", experiment, "_inter_discordant_pairs_unique_processed_blast_next_next.tsv"))
          write_tsv(blast_df_file_2_prev_prev_prev, file = paste0(strain, "/", sample, "_", experiment, "_inter_discordant_pairs_unique_processed_blast_prev_prev_prev.tsv"))
          write_tsv(blast_df_file_2_next_next_next, file = paste0(strain, "/", sample, "_", experiment, "_inter_discordant_pairs_unique_processed_blast_next_next_next.tsv")) 
...
for strain in "$MYWD"*/; do
  ...
  # Loop through sample prefixes (TSG, TLG, TLR)
    for sample in TSG TLG TLR; do
      for experiment in E1 E2 E3; do
        for pos in center prev next prev_prev next_next prev_prev_prev next_next_next; do
            ...
            (
                cd "$MYBLAST" || exit
                    ...
                    while IFS=$'\t' read -r nombre_read query_A reference_B || [ -n "$nombre_read" ]; do # -n to read last row also if it doesn't end with a newline
                        ...
                        blastn -db "$reference_name" -query "$file_name".fasta -dust no -outfmt "7 qseqid sseqid mismatch length" -out "$read_name"_"$reference_name"_randomid"$random_id"_results.tsv
...
for strain in "${MYWD}"*/; do
  for sample in TSG TLG TLR; do
    for experiment in E1 E2 E3; do
      ...
      file4="${strain}/${sample}_${experiment}_inter_discordant_pairs_unique_processed_blast_prev_prev_combined_results.tsv"
      file5="${strain}/${sample}_${experiment}_inter_discordant_pairs_unique_processed_blast_next_next_combined_results.tsv"
      file6="${strain}/${sample}_${experiment}_inter_discordant_pairs_unique_processed_blast_prev_prev_prev_combined_results.tsv"
      file7="${strain}/${sample}_${experiment}_inter_discordant_pairs_unique_processed_blast_next_next_next_combined_results.tsv"
      ...
      echo "CHECKING FILES EXIST:"
        [[ -f "$file1" ]] && echo "OK file1" || echo "MISSING file1 $file1"
        [[ -f "$file2" ]] && echo "OK file2" || echo "MISSING file2 $file2"
        [[ -f "$file3" ]] && echo "OK file3" || echo "MISSING file3 $file3"
        [[ -f "$file4" ]] && echo "OK file4" || echo "MISSING file4 $file4"
        [[ -f "$file5" ]] && echo "OK file5" || echo "MISSING file5 $file5"
        [[ -f "$file6" ]] && echo "OK file6" || echo "MISSING file6 $file6"
        [[ -f "$file7" ]] && echo "OK file7" || echo "MISSING file7 $file7"
        [[ -f "$fileorig" ]] && echo "OK fileorig" || echo "MISSING fileorig $fileorig"

      if [[ -f "$file1" && -f "$file2" && -f "$file3" && -f "$file4" && -f "$file5" && -f "$file6" && -f "$file7" && -f "$fileorig" && -f "$filecontrol" ]]; then
        echo "$(basename "$strain") ${sample} ${experiment}" >> "$log_file"
        echo "" >> "$log_file"  # Adds a blank line
        echo "Processing inter-discordant tsv files for $file1 , $file2 , $file3, $file4, $file5, $file6, $file7 and $fileorig" >> "$log_file"
        start_time=$SECONDS
        Rscript - <<EOF
          # load libraries
          ...
          path_to_file_4 <- "${file4}"
          path_to_file_5 <- "${file5}"   
          path_to_file_6 <- "${file6}"
          path_to_file_7 <- "${file7}"
          ...
          get_blast_validated_complete_df <- function (center_file, prev_file, next_file, prev_prev_file, next_next_file, prev_prev_prev_file, next_next_next_file) {
  
            complete_df <- full_join(prev_file, next_file, by = "pair_group") %>% 
              rename("valid_prev" = !!names(.[6]), "valid_next" = !!names(.[11])) %>%
              select(pair_group, valid_prev, valid_next) %>% 
              unique() %>% 
              full_join(prev_prev_file, ., by = "pair_group") %>% 
              rename("valid_prev_prev" = !!names(.[6])) %>% 
              select(pair_group, valid_prev, valid_next, valid_prev_prev) %>% 
              unique() %>% 
              full_join(next_next_file, ., by = "pair_group") %>% 
              rename("valid_next_next" = !!names(.[6])) %>% 
              select(pair_group, valid_prev, valid_next, valid_prev_prev, valid_next_next) %>%
              unique() %>% 
              full_join(prev_prev_prev_file, ., by = "pair_group") %>% 
              rename("valid_prev_prev_prev" = !!names(.[6])) %>%
              select(pair_group, valid_prev, valid_next, valid_prev_prev, valid_next_next, valid_prev_prev_prev) %>% 
              unique() %>% 
              full_join(next_next_next_file, ., by = "pair_group") %>% 
              rename("valid_next_next_next" = !!names(.[6])) %>%
              select(pair_group, valid_prev, valid_next, valid_prev_prev, valid_next_next, valid_prev_prev_prev, valid_next_next_next) %>% 
              unique() %>% 
              full_join(center_file, ., by = "pair_group") %>%
              rename("valid_center" = !!names(.[6])) %>% 
              select(pair_group, valid_prev, valid_next, valid_prev_prev, valid_next_next, valid_prev_prev_prev, valid_next_next_next, valid_center) %>% 
              unique() %>% 
              separate(pair_group, c("pair_group_name", "number"), "-", remove = FALSE) %>% 
              mutate_at(vars(valid_prev, valid_next, valid_prev_prev, valid_next_next, valid_prev_prev_prev, valid_next_next_next), ~replace_na(., "yes")) %>% 
              mutate(valid_all = ifelse(valid_center == "yes" & valid_prev == "yes" & valid_next == "yes" 
                                        & valid_prev_prev == "yes" & valid_next_next == "yes"
                                        & valid_prev_prev_prev == "yes" & valid_next_next_next == "yes", "yes", "no")) %>% 
          ...
          prev_prev_df <- process_blast_filtered_files(path_to_file_4)
          next_next_df <- process_blast_filtered_files(path_to_file_5)
          prev_prev_prev_df <- process_blast_filtered_files(path_to_file_6)
          next_next_next_df <- process_blast_filtered_files(path_to_file_7)
          ...
          valid_read_name <- get_blast_validated_complete_df(center_df, prev_df, next_df, prev_prev_df, next_next_df, prev_prev_prev_df, next_next_next_df)
...
#           ##### Cambiar en DI-GRAPH que estaba siempre path to file1!!!
...
#           next_df_option1 <- process_blast_filtered_files_option1(path_to_file_3)
#           next_df_option2 <- process_blast_filtered_files_option2(path_to_file_3)
#           next_df_option3 <- process_blast_filtered_files_option3(path_to_file_3)
#           next_df_option4 <- process_blast_filtered_files_option4(path_to_file_3)
#           next_df_option5 <- process_blast_filtered_files_option5(path_to_file_3)

#           prev_df_option1 <- process_blast_filtered_files_option1(path_to_file_2)
#           prev_df_option2 <- process_blast_filtered_files_option2(path_to_file_2)
#           prev_df_option3 <- process_blast_filtered_files_option3(path_to_file_2)
#           prev_df_option4 <- process_blast_filtered_files_option4(path_to_file_2)
#           prev_df_option5 <- process_blast_filtered_files_option5(path_to_file_2)

#           prev_prev_df_option1 <- process_blast_filtered_files_option1(path_to_file_4)
#           prev_prev_df_option2 <- process_blast_filtered_files_option2(path_to_file_4)
#           prev_prev_df_option3 <- process_blast_filtered_files_option3(path_to_file_4)
#           prev_prev_df_option4 <- process_blast_filtered_files_option4(path_to_file_4)
#           prev_prev_df_option5 <- process_blast_filtered_files_option5(path_to_file_4)

#           next_next_df_option1 <- process_blast_filtered_files_option1(path_to_file_5)
#           next_next_df_option2 <- process_blast_filtered_files_option2(path_to_file_5)
#           next_next_df_option3 <- process_blast_filtered_files_option3(path_to_file_5)
#           next_next_df_option4 <- process_blast_filtered_files_option4(path_to_file_5)
#           next_next_df_option5 <- process_blast_filtered_files_option5(path_to_file_5)

#           prev_prev_prev_df_option1 <- process_blast_filtered_files_option1(path_to_file_6)          
#           prev_prev_prev_df_option2 <- process_blast_filtered_files_option2(path_to_file_6)
#           prev_prev_prev_df_option3 <- process_blast_filtered_files_option3(path_to_file_6)
#           prev_prev_prev_df_option4 <- process_blast_filtered_files_option4(path_to_file_6)
#           prev_prev_prev_df_option5 <- process_blast_filtered_files_option5(path_to_file_6)

#           next_next_next_df_option1 <- process_blast_filtered_files_option1(path_to_file_7)
#           next_next_next_df_option2 <- process_blast_filtered_files_option2(path_to_file_7)
#           next_next_next_df_option3 <- process_blast_filtered_files_option3(path_to_file_7)
#           next_next_next_df_option4 <- process_blast_filtered_files_option4(path_to_file_7)
#           next_next_next_df_option5 <- process_blast_filtered_files_option5(path_to_file_7)   
