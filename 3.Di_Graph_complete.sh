...
# integrating a repair profiling,and a a genome wide coverage and discordant reads analysis.

# Keywords: 
# HO break, DNA repair, homologous recombination

# 150>75_SR_-v0_-m1 for the analysis of coverage at CHRIII, CHRV and MATa regions
# 150>18_SR_-v0_-k1 for the analysis of polymorphisms peaks at MAT regions

# Input files: 
# Assumes that the input files are gzipped fastq files.
...
# Uses 18nt reads nonRPMC -k1 alignment (net number of MATs polymorphis) for the analisys of the CHRIII/V polymorphisms ratio.
# Uses 75nt reads RPMC -m1 alignment for the MATs coverage
# Uses cutadapt for trimming, bowtie for alignment, and samtools and deeptools for post-processing.
# Average the coverage from three diferent experiments (E1, E2, E3) and generates TSV files with the average coverage for each genomic region.
...
# Integrate all data into a HTML and PDF report files.
      ...
      for experiment in E1 E2 E3; do
        ...
        if [[ -f "$file1_gz" && -f "$file2_gz" ]]; then
          ...
          if [[ -f "${subdir}/${sample}_${experiment}.fastq" ]]; then
            ...
            # Detecting number of cores availabe
            N_CPU=$(nproc)
