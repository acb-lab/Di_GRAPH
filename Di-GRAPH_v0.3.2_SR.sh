  ...
  for chr in CHRIII CHRV; do
    for suffix in "Coverage.tsv" "MATa_Coverage.tsv"; do
      ...
      if [[ -f "$file" ]]; then
        ...
        if ! Rscript "${MYRSCRIPTS}/${R_SCRIPT}" "$file" "$chr" "$suffix" "$subdir" 2>> "$log_file"; then
...
for subdir in "${MYWD}"*/; do
  for timepoint in T0 TSG TLG TLR; do
    ...
    if [[ -f "$file_chrIII" && -f "$file_chrV" ]]; then
      ...
      if ! Rscript "${MYRSCRIPTS}/${R_SCRIPT}" "$file_chrIII" "$file_chrV" "$timepoint" "$subdir" 2>> "$log_file"; then
