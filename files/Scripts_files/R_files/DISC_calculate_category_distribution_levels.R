#### R script to calculate valid inter-chromosomal discordant pairs category distribution (blast levels of validation)
...
log_step("Finding inter_discordant_pairs_unique_processed_valid_category_distribution files...")
# Get all coverage.tsv files recursively in root folder
discordant_reads_count_files <- list.files(
  path = root_dir,
  pattern = paste0(sample, ".*_", blast_option, ".*inter_discordant_pairs_unique_processed_valid_category_distribution\\.tsv$"),
  recursive = TRUE,
  full.names = TRUE
)

log_step("Processing inter_discordant_pairs_unique_processed_valid_category_distribution files...")
