#### R script to calculate valid inter-chromosomal discordant pairs matrix (merging all experiments)
...
if (length(args) != 4) {
  ...
  log_step("Usage: script.R <root_dir> <strain> <sample> <category_path>")
...
valid_reads_files <- list.files(
  path = root_dir,
  pattern = paste0(sample, ".*inter_discordant_pairs_unique_processed_valid\\.tsv$"),
  recursive = TRUE,
  full.names = TRUE
)
