#### R script to identify recombination hotspots (in all experiments, blast levels of validation)
...
if (length(args) != 5) {
  ...
  log_step("Usage: script.R <root_dir> <strain> <sample> <blast_option> <path_to_freqs>")
...
matrix_files <- list.files(
  path = root_dir,
  pattern = paste0(sample, ".*_", blast_option, ".*inter_discordant_pairs_unique_processed_valid_discordant_matrix\\.tsv$"),
  recursive = TRUE,
  full.names = TRUE
)
