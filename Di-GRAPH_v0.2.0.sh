...
Keywords: 
HO break, DNA repair, homologous recombination

150>75_SR_-v0_-m1 for the analysis of coverage at CHRIII, CHRV and MATa/MATa' regions
150>18_SR_-v0_-k1 for the analysis of polymorphisms peaks at MAT regions

Input files: 
Assumes that the input files are gzipped paired-end fastq files.
Uses RG_PMV_v9.fasta as reference genome, and is indexed with bowtie.
Name samples as T0_E1_R1.fastq.gz, T0_E1_R2.fastq.gz, T0_E2_R1.fastq.gz, T0_E2_R2.fasatq.gz, etc, etc. and put them in the same directory.
Analyses DNA repair at short/long timepoints (T0, TSG - TimeShortGal -, TLG -TimeLongGal- and TLR -TimeLongRaf-) using 3 replicates (E1, E2 and E3).

Specifications: 
This script processes paired-end fastq files, trims them to 75nt (or 18nt for the analysis of polymorphisms at MATs), aligns them as single read to a reference genome, and generates coverage files.
Reads are filtered for quality using fastp, Q30 quality along each nucleotide in the read.
Bedgraphs files are sorted before applying bedtools coverage to get the TSV files.
Uses 18nt reads nonRPGC -k1 alignment (net number of MATs polymorphis) for the analisys of the CHRIII/V polymorphisms ratio.
Uses 75nt reads RPGC -m1 alignment for the MATs coverage
Uses cutadapt for trimming, bowtie for alignment, and samtools and deeptools for post-processing.
Averages the coverage from three diferent experiments (E1, E2, E3) and generates TSV files with the average coverage for each genomic region.
MATa regions are generated by extracting coordinates coverage from the CHRIII/CHRV TSV files.
CHRIII and CHRV are binned into 1000bp before plotting and the average coverage is calculated. CHRIII/CHRV_MATs (75 and 18nt) are not binned.
For discordant analysis, this script processes paired-end fastq files, trims them to 75nt and adds _rA or _rB to read name after trimming,
...
Processes .sam output to keep only inter-chromosomal discordant read pairs and filters valid reads using BLAST

Generates plots using R-ggplot and saves it as a PNG and SVG files.
Generates a log_file with the processing timings and a stat file with the alignment information.

OPTIONS:
    -b/--blast          Path to BLAST features extraction directory (provided in GitHub)
    -c/--categories     Path to genomic categories directory (provided in GitHub)
